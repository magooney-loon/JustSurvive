...
{#if myChannel}
	...
	>
		<div
			style="font-size: 0.8rem; color: #f4a; margin-bottom: 0.4rem; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase;"
		>
			Reviving...
		</div>
		<div
			style="background: rgba(0,0,0,0.4); border-radius: 4px; height: 8px; width: 180px; overflow: hidden;"
		>
			<div
				style="background: #f4a; border-radius: 4px; height: 100%; width: {elapsed *
					100}%; transition: width 0.05s;"
			></div>
		</div>
	</div>
{/if}
